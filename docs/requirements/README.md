...
Altverfahrensevidenz, kein Sollmodell. Auch das
[5g-Kurzentscheidungs- und Freigabegate](../implementation/cemaris-increment-5g-completion.md)
endet mangels zuständiger Fach- und Freigabequellen mit Variante A. Der
...
intern datensparsam projiziert. Der
[5k-Abschluss](../implementation/cemaris-increment-5k-completion.md)
setzt die Projektentscheidung vom 25.08.2026 um: dauerhafter lokaler
SQL-Developmentbetrieb auf `Cemaris_Dev`, persistente Konten `admin` und `sach`
sowie ausschließlich die nicht personenbezogene EDWALT-
Friedhofsstammdatenmigration. Das
[5k-Quellmapping](../migration/edwalt-cemetery-master-data-mapping.md)
dokumentiert die tatsächlich belegten Felder und Sperren. Der bisherige
Abbruchparitätsbefund bleibt als technischer Providerparitätsnachweis
enthalten.
Vor jeder Gebühren-, Bescheid- oder Dokumentimplementierung war das
ausschließlich dokumentarische
[Entscheidungsgate 6a](../implementation/cemaris-increment-6a-completion.md)
auszuführen. Es ist mit Variante A „noch keine Implementierung“ abgeschlossen.
Die vollständigen
...
only und nicht produktiv freigegeben. Das
[Betriebs- und Pilotfreigabegate](../implementation/cemaris-notice-generation-pilot-release-gate-completion.md)
ist mit Variante A „Stop“ abgeschlossen. Die vollständige
[Pilotfreigabe-Entscheidungsakte](notice-generation-pilot-release-decisions.md)
dokumentiert den Widerspruch zwischen der gewünschten Nutzung von
dokumentiert die ergänzende Klarstellung, dass `Cemaris_Dev` eigens als
kombinierte Development- und Testpilotdatenbank geschaffen wurde, sowie alle
weiterhin offenen Nachweise. Der frühere Datenbankwiderspruch ist korrigiert;
es entstand weiterhin kein Aktivierungsauftrag.
...
Ausgestaltung bleiben `OFFEN`. Variante A aus 6a lässt den optionalen
DMS-Adapter und diese offenen Grenzen unverändert.
...
Die Gebühren-, Bescheid- und Dokumentfragen 6A-01 bis 6A-16 sowie die sechs
benötigten Entscheidungs- und Freigabepakete stehen gebündelt in den
[6a-Entscheidungen](fee-notice-document-decisions.md#gebündelte-entscheidungs--und-freigabeliste).
...
| Betriebs- und Pilotfreigabegate 6c-Pilot | technisch abgeschlossener, standardmäßig deaktivierter 6c-Pfad und installationsbezogene Betriebsnachweise | für genau eine benannte isolierte Umgebung entscheiden, ob ein rein synthetischer Pilot separat aktiviert und abgenommen werden darf | `Cemaris_Dev` als kombinierte Development-/Testpilotdatenbank und versionierte 23-Token-Fixture bestätigt; LibreOffice `26.8.0.3` installiert und startfähig; aktueller 6c-Migrationsstand, Restore-Protokoll, reale Ausgabe- und weitere Abnahmenachweise offen | keine Aktivierung, Konfigurationsänderung, echte Daten oder technische Erweiterung im Gate | abgeschlossen mit Variante A „Stop“ | [Entscheidungsakte](notice-generation-pilot-release-decisions.md), [Abschluss](../implementation/cemaris-notice-generation-pilot-release-gate-completion.md) und [Readiness-Folgeauftrag](../implementation/cemaris-notice-generation-synthetic-pilot-readiness-next-step-handoff.md); Capability bleibt aus |
