...
| REQ-DEV-001 | BESTÄTIGT | Für den dauerhaften lokalen Development-Zustand steht die bereits vorhandene SQL-Server-Datenbank `CEMARISDEV` zur Verfügung. | ausschließlich lokale Entwicklung | Das Anwendungsschema, lokale Konten, synthetische Personen-/Falldaten und freigegebene Friedhofsstammdaten bleiben über Neustarts erhalten; automatisierte SQL-Tests verwenden davon getrennte temporäre Datenbanken. | lokale Verbindungsdetails bleiben maschinenbezogen und geheim |
| REQ-DEV-002 | BESTÄTIGT | Die dauerhaften lokalen Benutzernamen sind `admin` und `sach`. | lokale Development-Datenbank `CEMARISDEV` | `admin` erhält `Administration`, `sach` erhält `Sachbearbeitung`; beide werden einmalig mit sicheren extern bereitgestellten Passwörtern angelegt und bei normalen Starts, Seeds und Importen erhalten. | konkrete Passwörter werden nicht dokumentiert oder eingebaut |
