...
| REQ-DEV-001 | BESTÄTIGT | Für den dauerhaften lokalen Development-Zustand steht die bereits vorhandene SQL-Server-Datenbank `Cemaris_Dev` zur Verfügung. | ausschließlich lokale Entwicklung | Das Anwendungsschema, lokale Konten, synthetische Personen-/Falldaten und freigegebene Friedhofsstammdaten bleiben über Neustarts erhalten; automatisierte SQL-Tests verwenden davon getrennte temporäre Datenbanken. | lokale Verbindungsdetails bleiben maschinenbezogen und geheim |
| REQ-DEV-002 | BESTÄTIGT | Die dauerhaften lokalen Benutzernamen sind `admin` und `sach`. | lokale Development-Datenbank `Cemaris_Dev` | `admin` erhält `Administration`, `sach` erhält `Sachbearbeitung`; beide werden einmalig mit extern bereitgestellten lokalen Passwortwerten angelegt und bei normalen Starts, Seeds und Importen erhalten. Für das lokale `admin`-Testkennwort ist die fehlende Repository-Unabhängigkeit ausschließlich im bestätigten isolierten 5k-Testbetrieb als Restrisiko akzeptiert. | konkrete Passwörter werden nicht dokumentiert; vor Weitergabe, zusätzlichem Zugriff oder produktiver Nutzung ist die Abweichung zwingend zu beseitigen |
