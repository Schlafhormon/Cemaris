...
EDWALT wird aktuell produktiv von ungefähr drei Personen verwendet. Die beiden
untersuchten Verzeichnisse enthalten nach fachlicher Auskunft das aktuelle
Programm und den aktuellen Datenstand (`INT-005`, `BESTÄTIGT`, Konfidenz hoch).
Dies bestätigt die Aktualität der Quelle, noch nicht ihre logische Konsistenz
für einen späteren Migrationsstichtag oder die Nutzung einzelner Funktionen.
...
|---|---|---|---|---|---|---|
| Fachliche DAT/IDX-Paare | 24 vollständige Paare, keine verwaisten DAT/IDX | wahrscheinlich primäre strukturierte Altbestände; Satzlayout/Schlüssel unbekannt | DAT-001 bis DAT-024 | ANNAHME | hoch | Welche Bestände sind führend, historisch, optional oder redundant? |
...
| Krematorium | W080 und KREMA/P080/P081 | eigener sensibler Fall-/Statusbereich; heute nicht genutzt, aber strukturierter historischer Bestand ist zu migrieren | DAT-021, FUN-400 ff., INT-008/027, REQ-MIG-003 | Inhalt `ANNAHME`, Nichtnutzung und Migrationsumfang `BESTÄTIGT` | hoch | Satzlayout, Feldsemantik, Schlüssel, Beziehungen und zulässige Datenkategorien? |
...
|---|---|---|---|---|---|
| MIG-R01 | Undokumentiertes proprietäres DAT/IDX-Satzlayout | keine verlässliche Extraktion, Typisierung oder Schlüsselprüfung | alle DAT/IDX, TECH-004 | sehr hoch; OFFEN; hoch | Copybooks/Herstellerdokumentation oder autorisierten garantiert lesenden Export beschaffen. |
...
| MIG-R03 | Alt-, DM- und alternative Varianten ohne Abgrenzung | Dubletten, Lücken oder falsche Historie | DAT-002/003/011/020/022/024 | hoch; ANNAHME; mittel | fachliche Archiv- und Vorrangregel pro Familie erheben. |
...
| MIG-R10 | Zeichensatz, Datums-, Währungs- und Rundungssemantik unbekannt | Umlaut-/Datumsfehler und finanzielle Differenzen | Windows-1252-Hilfen, DM-Bestände, Währungs-/USt-INI | hoch; ANNAHME; mittel | erst am autorisierten logischen Export profilieren. |
...
## Datenqualität: derzeit nur prüfbare Dimensionen

Aus den Originaldateien wurden bewusst keine Werte und keine Datensatzanzahlen
gelesen. Deshalb sind derzeit nur folgende technische Aussagen möglich:

- alle 24 DAT-Dateien besitzen einen gleichnamigen IDX-Partner;
- Größen und Zeitstempel unterscheiden aktuelle, historische und sehr kleine
  Bestände, ohne deren fachliche Qualität zu beweisen;
- W001 und W004 haben voneinander abweichende DAT-/IDX-Zeitstempel; dies ist
  kein Beweis für einen fehlerhaften Index;
- `STAT.DAT` im Handbuch und `STATIST.DAT` im Bestand bilden einen
  Namenswiderspruch;
- Satzanzahl, Nullquote, Längen, Wertebereiche, Dubletten,
  Referenzintegrität, gelöschte Sätze und Zeichensatz sind `OFFEN`.

Evidenz: DAT-001 bis DAT-024 und MAN-EDW-316. Status: `OFFEN` für
Inhaltsqualität, Konfidenz hoch für die technische Grenze. Offene Frage:
Welcher autorisierte logische Export erlaubt eine datensparsame Profilierung
mit Anzahlen, Längen, Nullquoten und maskierten Werteklassen?
...
2. unveränderliche Arbeitskopie außerhalb der Produktivverzeichnisse mit
   Hashnachweis;
3. dokumentiertes, garantiert lesendes Verfahren; keine Rebuild-/Reorg-
   Funktion und kein unbekannter schreibender Treiber;
...
Derzeit nicht belastbar ableitbar sind:

- ein endgültiges Cemaris-Datenmodell oder fachliche Entities;
- Feld-zu-Feld-Mappings oder transformierende Regeln;
- führende Schlüssel und Kardinalitäten;
- vollständige Datensatzanzahlen und Migrationsvolumen;
...
Aufbewahrung zu klären. Die Satzlayouts und Schlüsselbeziehungen der 24
DAT/IDX-Paare müssen vor einem Import sicher erschlossen werden. Eine
